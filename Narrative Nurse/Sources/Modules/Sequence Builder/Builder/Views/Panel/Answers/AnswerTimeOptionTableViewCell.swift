...
final class AnswerTimeOptionTableViewCell: UITableViewCell, AnswerOptionCell {
    ...
    var editedOption: Option? {
        ...
        return Option(kind: .text(title: self.titleTextField.text ?? ""), narrative: self.narrativeTextField.text ?? option.narrative, id: option.id)
    }
    ...
    private func setupViews() {
        ...
        let stackView = UIStackView(arrangedSubviews: [self.titleTextField, self.narrativeTextField])
    ...
    private func updateViews() {
        guard let option = self.option, case Option.Kind.text(let title) = option.kind else { return }
